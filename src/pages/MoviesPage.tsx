...
import { fetchMovies, INDIAN_LANGUAGES } from '../services/tmdb';
...
const MoviesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadMovies = async () => {
      setLoading(true);
      try {
        const data = await fetchMovies(page, genreParam, sortParam, yearParam, langParam);
        if (isMounted) {
          if (page === 1) {
        ...
        console.warn('Error loading movies from TMDB:', e);
      } finally {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-zinc-400 text-xs sm:text-sm mt-1">
              Explore Bollywood, Tollywood, Kollywood, Malayalam cinema, and international releases in 4K HDR.
            </p>
          </div>

          {/* Filter Bar with Apple Frosted Glass Dropdowns */}
        ...
        {loading && page > 1 && (
          <div className="flex items-center justify-center py-12">
            <div className="flex items-center gap-3 bg-black/60 px-5 py-3 rounded-full border border-white/10 backdrop-blur-xl shadow-lg">
              ...
              <span className="text-xs font-semibold text-zinc-300 tracking-wide">Loading more movies...</span>
            </div>
          </div>
        )}
